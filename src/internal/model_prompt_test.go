...
func testBasicPromptFunctionality(t *testing.T, dir1 string) {
	t.Run("Basic Prompt Opening", func(t *testing.T) {
		...
		TeaUpdate(m, utils2.TeaRuneKeyMsg(common.Hotkeys.OpenCommandLine[0]))
		...
		TeaUpdate(m, utils2.TeaRuneKeyMsg(common.Hotkeys.CancelTyping[0]))
		assert.False(t, m.promptModal.IsOpen())
		TeaUpdate(m, utils2.TeaRuneKeyMsg(common.Hotkeys.OpenSPFPrompt[0]))
		assert.True(t, m.promptModal.IsOpen())
	...
	t.Run("Shell command execution", func(t *testing.T) {
		...
		TeaUpdate(m, utils2.TeaRuneKeyMsg("mkdir test_dir"))
		...
		TeaUpdate(m, utils2.TeaRuneKeyMsg("xyz_non_exisiting_command"))
		TeaUpdate(m, tea.KeyMsg{Type: tea.KeyEnter})
	...
	t.Run("Model closing", func(t *testing.T) {
		...
		for _, key := range common.Hotkeys.CancelTyping {
			TeaUpdate(m, utils2.TeaRuneKeyMsg(common.Hotkeys.OpenSPFPrompt[0]))
			assert.True(t, m.promptModal.IsOpen())
			TeaUpdate(m, utils2.TeaRuneKeyMsg(key))
...
func testPanelOperations(t *testing.T, dir1, dir2, curTestDir string) {
	t.Run("Split Panel", func(t *testing.T) {
		...
		for len(m.fileModel.FilePanels) < m.fileModel.MaxFilePanel {
			...
			TeaUpdate(m, utils2.TeaRuneKeyMsg(prompt.SplitCommand))
			TeaUpdate(m, tea.KeyMsg{Type: tea.KeyEnter})
	...
	t.Run("cd Panel", func(t *testing.T) {
		...
		TeaUpdate(m, utils2.TeaRuneKeyMsg(common.Hotkeys.OpenSPFPrompt[0]))
		TeaUpdate(m, utils2.TeaRuneKeyMsg(prompt.CdCommand+" "+dir2))
		TeaUpdate(m, tea.KeyMsg{Type: tea.KeyEnter})
		...
		TeaUpdate(m, utils2.TeaRuneKeyMsg(prompt.CdCommand+" .."))
		TeaUpdate(m, tea.KeyMsg{Type: tea.KeyEnter})
		...
		TeaUpdate(m, utils2.TeaRuneKeyMsg(prompt.CdCommand+" "+filepath.Base(dir2)))
		TeaUpdate(m, tea.KeyMsg{Type: tea.KeyEnter})
		assert.True(t, m.promptModal.LastActionSucceeded(), "cd using relative path should work")
		assert.Equal(t, dir2, m.getFocusedFilePanel().Location)

		TeaUpdate(m, utils2.TeaRuneKeyMsg(prompt.CdCommand+" "+filepath.Join(dir2, "non_existing_dir")))
		TeaUpdate(m, tea.KeyMsg{Type: tea.KeyEnter})
		assert.False(t, m.promptModal.LastActionSucceeded(), "cd invalid abs path should not work")
		assert.Equal(t, dir2, m.getFocusedFilePanel().Location)

		TeaUpdate(m, utils2.TeaRuneKeyMsg(prompt.CdCommand+" non_existing_dir"))
		TeaUpdate(m, tea.KeyMsg{Type: tea.KeyEnter})
	...
	t.Run("open Panel", func(t *testing.T) {
		...
		TeaUpdate(m, utils2.TeaRuneKeyMsg(common.Hotkeys.OpenSPFPrompt[0]))
		TeaUpdate(m, utils2.TeaRuneKeyMsg(prompt.OpenCommand+" "+dir2))
		TeaUpdate(m, tea.KeyMsg{Type: tea.KeyEnter})
		...
		TeaUpdate(m, utils2.TeaRuneKeyMsg(prompt.OpenCommand+" ~"))
		TeaUpdate(m, tea.KeyMsg{Type: tea.KeyEnter})
		...
		TeaUpdate(m, utils2.TeaRuneKeyMsg(prompt.OpenCommand+fmt.Sprintf(" ${%s}", userHomeEnv)))
		TeaUpdate(m, tea.KeyMsg{Type: tea.KeyEnter})
		...
		TeaUpdate(m, utils2.TeaRuneKeyMsg(prompt.OpenCommand+" $(echo \"~\")"))
		TeaUpdate(m, tea.KeyMsg{Type: tea.KeyEnter})
		...
		TeaUpdate(m, utils2.TeaRuneKeyMsg(prompt.OpenCommand+" non_existing_dir"))
		TeaUpdate(m, tea.KeyMsg{Type: tea.KeyEnter})
		assert.False(t, m.promptModal.LastActionSucceeded(), "open using invalid relative path should not work")

		TeaUpdate(m, utils2.TeaRuneKeyMsg(prompt.OpenCommand+" "+filepath.Join(dir2, "non_existing_dir")))
		TeaUpdate(m, tea.KeyMsg{Type: tea.KeyEnter})
...
func testDirectoryHandlingWithQuotes(t *testing.T, curTestDir, dir1 string) {
	t.Run("Directory names with spaces and quotes", func(t *testing.T) {
		...
		utils2.SetupDirectories(t, directoriesToCreate...)

		t.Run("cd with double quotes", func(t *testing.T) {
			m := defaultTestModel(dir1)
			TeaUpdate(m, utils2.TeaRuneKeyMsg(common.Hotkeys.OpenSPFPrompt[0]))

			TeaUpdate(m, utils2.TeaRuneKeyMsg(prompt.CdCommand+` "`+dirWithSpaces+`"`))
			TeaUpdate(m, tea.KeyMsg{Type: tea.KeyEnter})
		...
		t.Run("cd with single quotes in path using double quotes", func(t *testing.T) {
			...
			TeaUpdate(m, utils2.TeaRuneKeyMsg(prompt.CdCommand+` "`+dirWithQuotes+`"`))
			TeaUpdate(m, tea.KeyMsg{Type: tea.KeyEnter})
		...
		t.Run("cd with double quotes in path using single quotes", func(t *testing.T) {
			...
			TeaUpdate(m, utils2.TeaRuneKeyMsg(prompt.CdCommand+` '`+dirWithSpecialChars+`'`))
			TeaUpdate(m, tea.KeyMsg{Type: tea.KeyEnter})
		...
		t.Run("cd with escaped spaces", func(t *testing.T) {
			...
			TeaUpdate(
				m,
				utils2.TeaRuneKeyMsg(prompt.CdCommand+` `+strings.ReplaceAll(dirWithSpaces, " ", `\ `)),
			)
			TeaUpdate(m, tea.KeyMsg{Type: tea.KeyEnter})
		...
		t.Run("open with double quotes", func(t *testing.T) {
			...
			TeaUpdate(m, utils2.TeaRuneKeyMsg(prompt.OpenCommand+` "`+dirWithSpaces+`"`))
			TeaUpdate(m, tea.KeyMsg{Type: tea.KeyEnter})
		...
		t.Run("cd with quoted environment variable", func(t *testing.T) {
			...
			TeaUpdate(m, utils2.TeaRuneKeyMsg(prompt.CdCommand+` "${`+userHomeEnv+`}"`))
			TeaUpdate(m, tea.KeyMsg{Type: tea.KeyEnter})
...
func testShellCommandsWithQuotes(t *testing.T, curTestDir, dir1 string) {
	t.Run("Shell command with quotes", func(t *testing.T) {
		...
		utils2.SetupDirectories(t, dirWithSpaces)

		t.Run("shell command with double quotes", func(t *testing.T) {
			m := defaultTestModel(dir1)
			TeaUpdate(m, utils2.TeaRuneKeyMsg(common.Hotkeys.OpenCommandLine[0]))

			TeaUpdate(m, utils2.TeaRuneKeyMsg(`mkdir "`+filepath.Join(dir1, "new dir with spaces")+`"`))
			TeaUpdate(m, tea.KeyMsg{Type: tea.KeyEnter})
		...
		t.Run("shell command with single quotes", func(t *testing.T) {
			...
			TeaUpdate(m, utils2.TeaRuneKeyMsg(`mkdir '`+filepath.Join(dir1, "another dir with spaces")+`'`))
			TeaUpdate(m, tea.KeyMsg{Type: tea.KeyEnter})
