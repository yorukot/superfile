...
func TestCopy(t *testing.T) {
	...
	t.Run("Basic Copy", func(t *testing.T) {
		...
		m := defaultModelConfig(false, false, []string{dir1})
		firstUse = false 

		// Todo validate current panel is "dir1"
		// Todo : Move all basic validation to a separate test
		// Everthing that doesn't have anything to do with copy paste
		...
		_, err = TeaUpdate(&m, utils.TeaRuneKeyMsg(common.Hotkeys.CopyItems[0]))
		...
		fmt.Println(m.fileModel.filePanels[m.filePanelFocusIndex].location)
		_, err = TeaUpdate(&m, utils.TeaRuneKeyMsg(common.Hotkeys.PasteItems[0]))
		...
		// Todo replace with slice exists check
		assert.Equal(t, "file1.txt",
			m.fileModel.filePanels[m.filePanelFocusIndex].element[0].name) 
