	...
	for _, tt := range testdata {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.shouldClipboardClear {
				// Wait for the Paste Model messsage to reach to Model via bubble tea event loop, that will clear clipboard
				// TODO: There are two eventually here in this test. A bit ineffiecient.
				// We have different asyc activities, create of files, completion of process message reaching processbar, model msg reaching model. Ideally we would like to have one final check, that is process completion message reaching model, and then check all conditions serially one be one.
