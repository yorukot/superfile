...
Count of extra column in file panel in addition to file name.

`0` => Extra columns feature is disabled

`n` (where n > 0) => Max count of extra columns (f.e. file size column)

- ###### file_panel_name_percent

If the percentage file name column is smaller than this number, the last additional column in list extra columns will be hidden. Minimal value is 25 maximal is 100.
