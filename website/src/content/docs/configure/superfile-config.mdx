...
Controls whether the shell prompt closes automatically after a command succeeds.

`true` => Close the shell after successful command execution

`false` => Keep the shell open after successful command execution
...
Even though the extra columns are enabled, they will be hidden if the size of the filepanel is small. Please either increase the size or adjust the file_panel_name_percent config if you don't see the columns.
...
Shows more detailed metadata for files and directories.

`true` => Enable detailed metadata. Requires [`exiftool`](https://exiftool.org).

`false` => Use the built-in metadata only.

- ###### enable_md5_checksum

Adds an MD5 checksum entry to metadata for regular files.

`true` => Calculate and show MD5 checksums in the metadata panel.

`false` => Do not calculate MD5 checksums.

:::caution

Calculating checksums reads the selected file, so it may be slow for large files.

:::

- ###### zoxide_support

Enables the zoxide navigation modal.

`true` => Enable zoxide navigation. Requires [`zoxide`](https://github.com/ajeetdsouza/zoxide).

`false` => Disable zoxide navigation.
...
- ###### show_folder_hotkey_hints

`true` (default) => Show right-aligned folder shortcut hints while Alt is held. Terminals without modifier press/release support show hints permanently.

`false` => Hide hints while keeping folder navigation shortcuts enabled.

Hints are always hidden in prompts, searches, rename fields, and dialogs.
