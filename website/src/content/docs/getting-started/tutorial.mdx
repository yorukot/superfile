...
Press `Z` (shift+z) to start recursive search mode. The focused panel switches to a live, fuzzy match over the whole tree below the current directory, ranked by relevance. Type to refine the query (fzf extended syntax is supported, e.g. `^main`, `\.go$`, `!test`). Use `list_up`/`list_down`/`page_up`/`page_down` bindings (default `up`/`down`/`pgup`/`pgdown`; bare letters always type, so add e.g. `ctrl+n` to `list_down` for vim-style scrolling) to move through the result set. Press `alt`+`.` to toggle hidden files without leaving search (your query is preserved; the search rewalks with the new visibility). Note: `ctrl`+`.` is intentionally unsupported — most terminals cannot deliver `ctrl`+punctuation combos and would type the character into the query instead. Press `enter` on a file result to open it, or on a directory result to make it the new current directory. Press `esc` or `ctrl`+`c` to cancel and return to the directory listing.
...
This may become tedious when you have a large number of items. Instead, you can repeatedly press `shift`+`up` or `K` (shift+k) to select items while moving the cursor upward, or `shift`+`down` or `J` (shift+j) to select items while moving the cursor downward.
